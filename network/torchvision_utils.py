...
from typing import Any, BinaryIO, List, Optional, Tuple, Union, Callable, Dict, TypeVar, Sequence, Mapping
...
import torchvision.transforms.functional as torchvision_transforms_F
...
def draw_bounding_boxes(
    ...
    if colors is None:
        colors = _generate_color_palette(num_boxes)  # type: ignore
    ...
    colors = [(ImageColor.getrgb(color) if isinstance(color, str) else color) for color in colors]  # type: ignore
...
def draw_segmentation_masks(
    ...
    if colors is None:
        colors = _generate_color_palette(num_masks)  # type: ignore

    if not isinstance(colors, list):
        colors = [colors]  # type: ignore
    if not isinstance(colors[0], (tuple, str)):  # type: ignore
        raise ValueError("colors must be a tuple or a string, or a list thereof")
    if isinstance(colors[0], tuple) and len(colors[0]) != 3:  # type: ignore
...
def _normalized_flow_to_image(normalized_flow: torch.Tensor) -> torch.Tensor:
    ...
    flow_image = torch.zeros((N, 3, H, W), dtype=torch.uint8, device=device) # type: ignore
    colorwheel = _make_colorwheel().to(device)  # shape [55x3]
    num_cols = colorwheel.shape[0]
    norm = torch.sum(normalized_flow**2, dim=1).sqrt()
    a = torch.atan2(-normalized_flow[:, 1, :, :], -normalized_flow[:, 0, :, :]) / math.pi  # type: ignore
    fk = (a + 1) / 2 * (num_cols - 1)
...
def _make_ntuple(x: Any, n: int) -> Tuple[Any, ...]:
    ...
    """
    if isinstance(x, collections.abc.Iterable):  # type: ignore
        return tuple(x)
    ...
    def __init__(
        ...
        if padding is None:
            if isinstance(kernel_size, int) and isinstance(dilation, int):
                ...
                _conv_dim = len(kernel_size) if isinstance(kernel_size, Sequence) else len(dilation)  # type: ignore
...
class ImageClassification(nn.Module):
    ...
    def forward(self, img: Tensor) -> Tensor:
        img = torchvision_transforms_F.resize(img, self.resize_size, interpolation=self.interpolation, antialias=self.antialias)  # type: ignore
        img = torchvision_transforms_F.center_crop(img, self.crop_size)
        if not isinstance(img, Tensor):
            img = torchvision_transforms_F.pil_to_tensor(img)
        img = torchvision_transforms_F.convert_image_dtype(img, torch.float)
        img = torchvision_transforms_F.normalize(img, mean=self.mean, std=self.std)
        return img
    ...
    def outer_wrapper(builder: Callable[..., M]) -> Callable[..., M]:
        ...
        def inner_wrapper(*args: Any, **kwargs: Any) -> M:
            for weights_param, (pretrained_param, default) in weights.items():  # type: ignore[union-attr]
                ...
                if pretrained_arg:
                    msg = (
                        f"{msg} You can also use `{weights_param}={type(default_weights_arg).__name__}.DEFAULT` "  # type: ignore
